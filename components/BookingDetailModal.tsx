...
export function BookingDetailModal({ visible, onClose, lead, bookingData, itineraries, onSuccess, styles }: {
  visible: boolean;
  onClose: () => void;
  lead: Lead | null;
  bookingData: any;
              ...
              {bookingData?.checklist && (
                <View style={[styles.bookCard, { borderColor: '#10b98144', backgroundColor: '#10b98108' }]}>
                  ...
                  <View style={{ height: 6, backgroundColor: '#1e293b', borderRadius: 3, overflow: 'hidden' }}>
                    <View style={{ height: '100%', backgroundColor: '#10b981', width: (Object.values(bookingData.checklist).filter(v => !!v).length / 7) * 100 + '%' }} />
                  </View>
                </View>
              )}
                ...
                <>
                  <View style={styles.bookCard}>
                    ...
                    <View style={styles.bookGrid}>
                      <View style={styles.bookGridItem}><Text style={styles.bookGridLabel}>Total</Text><Text style={styles.bookGridValue}>₹{bookingData.total_amount?.toLocaleString()}</Text></View>
                      <View style={styles.bookGridItem}><Text style={styles.bookGridLabel}>Paid</Text><Text style={styles.bookGridValue}>₹{bookingData.advance_paid?.toLocaleString()}</Text></View>
                      <View style={styles.bookGridItem}><Text style={styles.bookGridLabel}>Due</Text><Text style={[styles.bookGridValue, { color: '#ef4444' }]}>₹{bookingData.due_amount?.toLocaleString()}</Text></View>
                    </View>
                  </View>

...
export default function OperationsDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    // Filter by ops_assigned_to for this user
  ...
  const handleSave = async () => {
    ...
    if (editingBooking.itinerary_id && editingBooking.itinerary_id !== selectedLead?.itinerary_id) {
      await supabase.from('leads').update({ 
        ...
        ops_itinerary_edited: true 
      }).eq('id', selectedLead?.id);
    }

    const { error } = await supabase.from('confirmed_bookings').update({
    ...
    const html = `
      <html>
        ...
        <body>
          ...
          <div class="footer">
            <div class="legal">Nomadller Pvt Limited • Trip Manifest v2.0 • Ops Copy</div>
      ...
      <View style={s.card}>
        ...
        <View style={s.cardContent}>
          <View style={s.cardHeader}>
            ...
            <View style={s.percentBadge}><Text style={s.percentText}>{progress}%</Text></View>
          </View>
          <TouchableOpacity style={s.opsBtn} onPress={() => {
            setSelectedLead(item);
            setEditingBooking(booking ? { ...booking } : null);
            setModalOpen(true);
          }}>
            <Ionicons name="settings-outline" size={16} color="#fff" />
            <Text style={s.opsBtnText}>MANAGE OPERATIONS</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
  ...
  return (
    <View style={s.container}>
      ...
      {loading ? <ActivityIndicator color="#6366f1" style={{ marginTop: 40 }} /> : (
        <FlatList data={filteredLeads} keyExtractor={i => i.id} renderItem={renderLeadCard} contentContainerStyle={s.list}
          ListEmptyComponent={<View style={s.emptyWrap}><Text style={s.emptyTitle}>No leads assigned to you.</Text></View>}
        />
      )}
      <Modal visible={modalOpen} animationType="slide">
        <View style={s.modal}>
          <View style={s.modalHeader}>
            ...
            <TouchableOpacity onPress={() => setModalOpen(false)}><Ionicons name="close" size={28} color="#94a3b8" /></TouchableOpacity>
          </View>
          <ScrollView contentContainerStyle={s.modalScroll}>
                ...
                <View key={item.key} style={s.checklistCard}>
                  <View style={s.checklistHeader}>
                    <TouchableOpacity onPress={() => toggleChecklist(item.key)}>
                      <Ionicons name={checked ? "checkbox" : "square-outline"} size={22} color={checked ? "#10b981" : "#64748b"} />
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity onPress={() => toggleExpand(item.key)}>
                      <Ionicons name={expandedItems[item.key] ? "chevron-up" : "chevron-down"} size={20} color="#64748b" />
                    </TouchableOpacity>
                  </View>
            ...
            <TouchableOpacity style={s.saveBtn} onPress={handleSave} disabled={saving}>
              {saving ? <ActivityIndicator color="#fff" /> : <Text style={s.saveBtnText}>Save Progress</Text>}
            </TouchableOpacity>
          </ScrollView>
        </View>
      </Modal>
  ...
  return (
    <View style={{ marginBottom: 10 }}>
      ...
      {showSug && filtered.length > 0 && (
        <View style={{ backgroundColor: '#1e293b', borderRadius: 8, marginTop: 4, overflow: 'hidden', borderWeight: 1, borderColor: '#334155' }}>
...
const s = StyleSheet.create({
  ...
  percentBadge: { backgroundColor: '#0f172a', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 },
  percentText: { color: '#cbd5e1', fontSize: 12, fontWeight: '800' },
  opsBtn: { backgroundColor: '#6366f1', borderRadius: 10, paddingVertical: 12, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8 },
  opsBtnText: { color: '#fff', fontSize: 13, fontWeight: '800' },
  modal: { flex: 1, backgroundColor: '#0f172a' },
  modalHeader: { padding: 20, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: '#1e293b' },
  modalTitle: { color: '#f8fafc', fontSize: 20, fontWeight: '800' },
  modalScroll: { padding: 20, gap: 16 },
  checklistCard: { backgroundColor: '#1e293b', borderRadius: 12, padding: 14, borderWidth: 1, borderColor: '#334155' },
  ...
  checklistContent: { marginTop: 10, gap: 8 },
  ...
  genBtn: { backgroundColor: '#10b981', padding: 12, borderRadius: 8, alignItems: 'center' },
